...
SiPMMessenger::SiPMMessenger(SiPM* sipm) : mySiPM(sipm) {
  ...
  shapeCmd->SetGuidance(
      "Set rise time, fall time and sampling of single pixel signal");
...
void SiPMMessenger::SetNewValue(G4UIcommand* command, G4String newValue) {
  ...
  if (command == pdeCmd) {
    ...
    G4cout << ">>>>> SiPM PDE is now " << pdeCmd->GetNewDoubleValue(newValue)
           << " <<<<<" << G4endl;
  }

  if (command == gainCmd) {
    mySiPM->GetGossip()->gain = gainCmd->GetNewDoubleValue(newValue);
    G4cout << ">>>>> SiPM gain is now " << gainCmd->GetNewDoubleValue(newValue)
           << " <<<<<" << G4endl;
  }

  if (command == tdrCmd) {
    mySiPM->GetGossip()->tau_dr = tdrCmd->GetNewDoubleValue(newValue);
    G4cout << ">>>>> SiPM tau_dr is now " << tdrCmd->GetNewDoubleValue(newValue)
           << " <<<<<" << G4endl;
  }

  if (command == apsCmd) {
    mySiPM->GetGossip()->Pap_s = apsCmd->GetNewDoubleValue(newValue);
    G4cout << ">>>>> SiPM Pap_s is now " << apsCmd->GetNewDoubleValue(newValue)
           << " <<<<<" << G4endl;
  }

  if (command == tapsCmd) {
    mySiPM->GetGossip()->tau_ap_s = tapsCmd->GetNewDoubleValue(newValue);
    G4cout << ">>>>> SiPM tau_ap_s is now "
           << tapsCmd->GetNewDoubleValue(newValue) << " <<<<<" << G4endl;
  }

  if (command == apfCmd) {
    mySiPM->GetGossip()->Pap_f = apfCmd->GetNewDoubleValue(newValue);
    G4cout << ">>>>> SiPM Pap_f is now " << apfCmd->GetNewDoubleValue(newValue)
           << " <<<<<" << G4endl;
  }

  if (command == tapfCmd) {
    mySiPM->GetGossip()->tau_ap_f = tapfCmd->GetNewDoubleValue(newValue);
    G4cout << ">>>>> SiPM tau_ap_f is now "
           << tapfCmd->GetNewDoubleValue(newValue) << " <<<<<" << G4endl;
  }

  if (command == xtCmd) {
    mySiPM->GetGossip()->Pxt = xtCmd->GetNewDoubleValue(newValue);
    G4cout << ">>>>> SiPM Pxt is now " << xtCmd->GetNewDoubleValue(newValue)
           << " <<<<<" << G4endl;
  }

  if (command == enfCmd) {
    mySiPM->GetGossip()->ENF = enfCmd->GetNewDoubleValue(newValue);
    G4cout << ">>>>> SiPM ENF is now " << enfCmd->GetNewDoubleValue(newValue)
           << " <<<<<" << G4endl;
  }

  if (command == enCmd) {
    mySiPM->GetGossip()->EN = enCmd->GetNewDoubleValue(newValue);
    G4cout << ">>>>> SiPM EN is now " << enCmd->GetNewDoubleValue(newValue)
           << " <<<<<" << G4endl;
  }
  ...
  if (command == npixCmd) {
    ...
    G4cout << ">>>>> SiPM Npx / Npy is now "
           << sqrt(npixCmd->GetNewDoubleValue(newValue)) << " / "
           << sqrt(npixCmd->GetNewDoubleValue(newValue)) << " <<<<<" << G4endl;
  }

  if (command == recoveryCmd) {
    mySiPM->GetGossip()->tau_recovery =
        recoveryCmd->GetNewDoubleValue(newValue);
    G4cout << ">>>>> SiPM tau_recovery is now "
           << recoveryCmd->GetNewDoubleValue(newValue) << " <<<<<" << G4endl;
  }
  ...
  if (command == shapeCmd) {
    ...
    G4cout << ">>>>> SiPM pulse shape is now: tau1 = " << tau1
           << ", tau2 = " << tau2 << ", sampling = " << sampling << " <<<<<"
           << G4endl;
  }

  if (command == signalAmplitudeCmd) {
    mySiPM->GetGossip()->signalAmp =
        signalAmplitudeCmd->GetNewDoubleValue(newValue);
    G4cout << ">>>>> SiPM signalAmp is now "
           << signalAmplitudeCmd->GetNewDoubleValue(newValue) << " <<<<<"
           << G4endl;
  }
